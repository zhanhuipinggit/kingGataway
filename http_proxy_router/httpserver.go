...
import (
	...
	"github.com/zhanhuipinggit/kingGataway/cert_file"
...
func HttpServerRun() {
	...
	log.Printf(" [INFO] http_proxy_server_run:%s\n",lib.GetStringConf("proxy.http.addr"))
	if err := HttpSrvHandler.ListenAndServe(); err != nil {
		log.Fatalf(" [ERROR] http_proxy_server_run:%s err:%v\n", lib.GetStringConf("proxy.http.addr"), err)
	}
}
...
func HttpsServerStop() {
	...
	if err := HttpsSrvHandler.Shutdown(ctx); err != nil {
		log.Fatalf(" [ERROR] https_proxy_server_stop err:%v\n", err)
	}
	log.Printf(" [INFO] https_proxy_server stopped\n")
}
...
func HttpServerStop() {
	...
	if err := HttpSrvHandler.Shutdown(ctx); err != nil {
		log.Fatalf(" [ERROR] http_proxy_server_stop err:%v\n", err)
	}
	log.Printf(" [INFO] http_proxy_server stopped\n")
}
